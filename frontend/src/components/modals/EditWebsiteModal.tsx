...
import { X, Globe, AlertCircle, Activity, ArrowRight, Sparkles, Clock, Zap, Edit } from 'lucide-react';
...
interface EditWebsiteModalProps {
  website: Website;
  onClose: () => void;
  onWebsiteUpdated: (website: Website) => void;
}

export default function EditWebsiteModal({ website, onClose, onWebsiteUpdated }: EditWebsiteModalProps) {
  const { user } = useUser();
  const { getToken } = useAuth();
  const [name, setName] = useState(website.name);
  const [url, setUrl] = useState(website.url);
  const [interval, setInterval] = useState<'1min' | '5min' | '30min'>(website.interval);
        ...
        >
          ...
          <div className="flex items-center justify-between p-8 border-b border-slate-200 bg-gradient-to-r from-slate-50 to-green-50/30">
            <div className="flex items-center space-x-4">
              ...
              <div>
                <h2 className="text-2xl font-bold text-slate-900">Edit Website</h2>
                <p className="text-slate-600">Update monitoring settings</p>
              </div>
            </div>
            <motion.button
              onClick={onClose}
          ...
          <form onSubmit={handleSubmit} className="p-8">
            ...
            <div className="space-y-8">
              ...
              <div className="space-y-6">
                ...
                {/* Current Stats */}
                <div className="bg-gradient-to-r from-purple-50 to-pink-50/30 rounded-2xl p-6 border border-slate-100">
                  <div className="flex items-center space-x-3 mb-4">
                    <div className="w-8 h-8 bg-gradient-to-br from-purple-500 to-pink-600 rounded-xl flex items-center justify-center">
                      <Activity className="w-4 h-4 text-white" />
                    </div>
                    <h3 className="text-lg font-semibold text-slate-900">Current Statistics</h3>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div className="bg-white/80 backdrop-blur-sm rounded-xl p-4 border border-slate-200">
                      ...
                      <p className="text-lg font-bold text-slate-900">{website.responseTime}ms</p>
                    </div>
                    <div className="bg-white/80 backdrop-blur-sm rounded-xl p-4 border border-slate-200">
                      <p className="text-sm text-slate-600 mb-1">Uptime</p>
                      <p className="text-lg font-bold text-slate-900">{website.uptime.toFixed(1)}%</p>
                    </div>
                    <div className="bg-white/80 backdrop-blur-sm rounded-xl p-4 border border-slate-200">
                      <p className="text-sm text-slate-600 mb-1">Last Check</p>
