...
import { X, Globe, AlertTriangle, AlertCircle, Trash2 } from 'lucide-react';
...
export default function DeleteWebsiteModal({ website, onClose, onWebsiteDeleted }: DeleteWebsiteModalProps) {
  const { user } = useUser();
        ...
        >
          ...
          <div className="flex items-center justify-between p-8 border-b border-slate-200 bg-gradient-to-r from-red-50 to-pink-50/30">
            <div className="flex items-center space-x-4">
              ...
              <div>
                <h2 className="text-2xl font-bold text-slate-900">Delete Website</h2>
            ...
            <div className="space-y-6">
              ...
              <div className="bg-gradient-to-r from-slate-50 to-blue-50/30 rounded-2xl p-6 border border-slate-100">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-white/80 backdrop-blur-sm rounded-xl p-4 border border-slate-200">
                    <p className="text-sm text-slate-600 mb-1">Website Name</p>
                    <p className="text-sm font-semibold text-slate-900 truncate">{website.name}</p>
                  </div>
                  <div className="bg-white/80 backdrop-blur-sm rounded-xl p-4 border border-slate-200">
                    <p className="text-sm text-slate-600 mb-1">Status</p>
                    <p className="text-sm font-semibold text-slate-900 capitalize">{website.status}</p>
                  </div>
                  <div className="bg-white/80 backdrop-blur-sm rounded-xl p-4 border border-slate-200">
                    <p className="text-sm text-slate-600 mb-1">Response Time</p>
                    <p className="text-sm font-semibold text-slate-900">{website.responseTime}ms</p>
                  </div>
                  <div className="bg-white/80 backdrop-blur-sm rounded-xl p-4 border border-slate-200">
                    <p className="text-sm text-slate-600 mb-1">Uptime</p>
                    <p className="text-sm font-semibold text-slate-900">{website.uptime.toFixed(1)}%</p>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-blue-50/50 rounded-xl p-4 border border-blue-200/50">
                <p className="text-sm text-blue-700">
                  <strong>Note:</strong> This action cannot be undone and all monitoring data will be permanently lost. Please make sure you have backed up any important information.
                </p>
              </div>
            </div>
